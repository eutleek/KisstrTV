...
/* 限流/风控专用错误码 —— 这些码代表「现在拿不到」，不代表「这条视频没了」：
     -1 响应不可解析（常为被拦截的 HTML 占位页）  -2 请求抛错（断网/DNS/超时）
     -412 请求被拦截（风控）  -509 超出限制  -799 请求过于频繁
   宿主给这类失败打上 net 标记，渲染层据此只跳过、绝不从曲库删除。
   不加这道区分，一次 412 限流就会把整个曲库当成"全是失效源"删空。 */
const BILI_NET_CODES = new Set([-1, -2, -412, -509, -799]);
function biliIsNet(d) {
  if (!d) return true;
  if (d.net) return true;
  return BILI_NET_CODES.has(Number(d.code));
}
async function biliApi(url, params) {
  try {
    ...
    // HTTP 层被拦截（412 风控 / 429 / 5xx）或返回体不是 JSON —— 一律按"网络受限"处理
    if (!j || typeof j !== 'object' || res.status === 412 || res.status === 429 || res.status >= 500) {
      return {
        code: (j && j.code != null) ? j.code : -1,
        message: (j && j.message) || ('响应不可用（HTTP ' + res.status + '）'),
        net: true
      };
    }
    if (BILI_NET_CODES.has(Number(j.code))) j.net = true;
    return j;
  } catch (e) {
    return { code: -2, message: '网络错误：' + (e && e.message ? e.message : e), net: true };
  }
}
...
async function biliResolve(bvid) {
  ...
  // 网络/风控层失败（412 限流、断网、超时）≠ 稿件失效 —— 打 net 标记，渲染层只跳过不删库
  if (biliIsNet(d)) return { ok: false, net: true, msg: 'B站暂时取不到（限流或断网），本条保留待重试' };
  // 能走到这里说明服务端明确回答了 code != 0 —— 这才是真正的源失效
  if (!d || d.code !== 0) return { ok: false, msg: '视频不存在或不可播放' };
  const data = d.data;
  if (!data) return { ok: false, net: true, msg: '接口返回结构异常，本条保留' };
  const cid = data.pages && data.pages[0] ? data.pages[0].cid : 0;
  if (!cid) return { ok: false, net: true, msg: '无法获取 cid，本条保留' };
  for (const qn of biliQnOrder()) {
    ...
    if (biliIsNet(p)) return { ok: false, net: true, msg: 'B站暂时取不到（限流或断网），本条保留待重试' };
  }
  // 所有清晰度都拿不到 = 权限/登录问题（版权受限、需登录），与源是否有效无关，绝不据此删库
  return {
    ok: false, net: true,
    msg: biliCookie ? '该视频清晰度受限' : '未登录时仅提供低清晰度，建议登录B站获取高清'
  };
}
/* 获取视频分P列表（用于把「OP/ED合集」拆成单曲电台） */
async function biliPages(bvid) {
  const d = await biliApi('https://api.bilibili.com/x/web-interface/view', { bvid });
  if (biliIsNet(d)) return { ok: false, net: true, msg: '合集信息暂时取不到（限流或断网）' };
...
async function biliResolveCid(bvid, cid, part) {
  if (!cid) return { ok: false, net: true, msg: 'cid 无效' };
  for (const qn of biliQnOrder()) {
    const p = await biliApi('https://api.bilibili.com/x/player/playurl', { bvid, cid, qn, fnval: 0, fnver: 0 });
    if (p && p.code === 0 && p.data && p.data.durl && p.data.durl.length) {
  ...
  return { ok: false, net: true, msg: biliCookie ? '该分P清晰度受限' : '未登录时仅提供低清晰度' };
}
...
/* 曲库读写安全加固：
   老逻辑读失败时把内存置成空库，此后任何一次 vaultAdd 都会把整个 vault.json
   覆盖成「只剩刚抓的几条」—— 一次解析失败就丢掉全部曲库。现在三道闸：
     ① vaultLoadOk 记录「是否真从磁盘读到过」，没读到过一律禁止写盘；
     ② 首次成功加载后留一份 vault.prev.json 快照；
     ③ 加载失败大声报错，不再静默显示 0 条。 */
let vaultLoadOk = false;
let vaultLoadErr = '';
function vaultBackupOnce() {
  try {
    const bak = VAULT_FILE.replace(/\.json$/, '.prev.json');
    if (!fs.existsSync(bak)) fs.copyFileSync(VAULT_FILE, bak);
  } catch (e) {}
}
function vaultLoad() {
  try {
    // vault.json 与 WebView2(C#) 宿主共用同一份文件，而 .NET 的 Encoding.UTF8 写盘会带
    // UTF-8 BOM（EF BB BF）。JSON.parse 不认 BOM，会直接抛 "Unexpected token" 导致
    // 曲库读成 0 条 —— 这里先剥掉 BOM 再解析，对两侧写的文件都兼容。
    let txt = fs.readFileSync(VAULT_FILE, 'utf8');
    ...
    if (d && Array.isArray(d.items)) {
      ...
      vaultLoadOk = true;
      vaultBackupOnce();
      console.log('[vault] loaded ' + d.items.length + ' items');
      return true;
    }
    vaultLoadErr = 'items 字段不是数组';
  } catch (e) {
    if (e && e.code === 'ENOENT') {        // 首次运行：文件不存在属正常状态
      vault = { items: [] };
      vaultLoadOk = true;
      return true;
    }
    vaultLoadErr = (e && e.message) ? e.message : String(e);
  }
  console.error('[vault] LOAD FAILED (' + vaultLoadErr + ') — 磁盘文件保持原样，已禁止写盘以免覆盖曲库');
  return false;
}
...
function vaultWrite() {
  if (!vaultLoadOk) {   // 没成功读到过磁盘 → 内存里的空库不可信，绝不覆盖磁盘
    console.error('[vault] save BLOCKED — never loaded successfully, disk file preserved');
    return false;
  }
  try { fs.writeFileSync(VAULT_FILE, JSON.stringify(vault)); return true; } catch (e) { return false; }
}
function vaultSave() {
  vaultDirty = true;
...
/* 失效源「真删除」（不再拉黑）：渲染层确认某条源确实打不开时调用，直接从曲库抹掉并立即落盘。
   ids 里的元素可能是 bvid（B站）或 mid（网易云 MV id）；曲库只按 bvid 索引，不匹配的自动忽略。
   注意：这条路径只该被「服务端明确回答稿件不存在」触发 —— 限流/断网类失败渲染层不会调到这里。 */
function vaultRemove(ids) {
  const set = new Set((Array.isArray(ids) ? ids : [])
    .map((x) => String(x == null ? '' : x).trim().toUpperCase())
    .filter(Boolean));
  if (!set.size) return { ok: true, removed: 0, total: vault.items.length };
  if (!vaultLoadOk) {   // 曲库没读成功时禁止删除：此刻内存是空库，删了会把磁盘也清空
    return { ok: false, removed: 0, msg: '曲库未成功加载，已跳过删除以保护数据' };
  }
  const before = vault.items.length;
  vault.items = vault.items.filter((x) => !set.has(String(x.bvid || '').toUpperCase()));
  const removed = before - vault.items.length;
  if (removed) { vaultDirty = true; vaultSave(); vaultFlush(); }   // 立即落盘，不等 1s 节流
  return { ok: true, removed, total: vault.items.length };
}
...
ipcMain.handle('vault-remove', (_e, ids) => vaultRemove(Array.isArray(ids) ? ids : (ids ? [ids] : [])));
...
async function neteaseResolve(mid) {
  ...
  const j = r.json;
  // 网易云侧的失败几乎全是登录态/版权态或会话页异常，与"这个 MV 没了"无关 ——
  // 一律打 net 标记（与宿主 C# 版 Netease.Resolve 的语义保持一致），只跳过不删库
  if (!j || j.code !== 200 || !j.data) {
    return { ok: false, net: true, msg: '网易云暂时取不到 MV（' + ((j && j.code) || r.status) + '，可能需登录或版权受限），本条保留' };
  }
  const brs = j.data.brs || {};
  let url = null;
  for (const br of ['1080', '720', '480', '240']) { if (brs[br]) { url = brs[br]; break; } }
  if (!url) return { ok: false, net: true, msg: '未找到可播放的 MV 地址，本条保留待重试' };
